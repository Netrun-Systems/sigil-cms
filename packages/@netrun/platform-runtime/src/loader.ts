 ...
 * Plugin Loader
 *
 * Loads and initializes platform plugins at startup.
 * Each plugin's register() is wrapped in try/catch so one
 * failing plugin doesn't break the rest of the application.
 */

import type { Router } from 'express';
import type { PlatformPlugin, PluginContext, PluginLogger, DrizzleClient } from './types.js';
    ...
    const ctx: PluginContext = {
      ...
      async runMigration(sqlStr: string) {
        try {
          ...
          await (db as any).execute(sqlTag.raw(sqlStr));
        } catch {
          // Fallback for environments where drizzle sql.raw isn't available
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          try { await (db as any).execute({ queryChunks: [sqlStr] }); } catch { /* migration optional */ }
        }
      },
