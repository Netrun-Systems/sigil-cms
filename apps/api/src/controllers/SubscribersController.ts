 ...
 *
 * Mailing list management — subscribe, unsubscribe, broadcast, CRUD.
 * CAN-SPAM / GDPR compliant with one-click unsubscribe.
 */

import type { Request, Response } from 'express';
import { eq, and, count, sql } from 'drizzle-orm';
import { subscribers, sites } from '@netrun-cms/db';
...
export class SubscribersController {
  ...
  static async broadcast(req: AuthenticatedRequest, res: Response): Promise<void> {
    const db = getDb();
    const { siteId } = req.params;
    ...
    const [site] = await db.select({ domain: sites.domain, name: sites.name }).from(sites).where(eq(sites.id, siteId)).limit(1);
  ...
  static async delete(req: AuthenticatedRequest, res: Response): Promise<void> {
    ...
    await db.delete(subscribers).where(and(eq(subscribers.id, id), eq(subscribers.siteId, siteId)));
  ...
  static async subscribe(req: Request, res: Response): Promise<void> {
    ...
    await db.insert(subscribers).values({ siteId: site.id, email: normalized, name: name || null });
