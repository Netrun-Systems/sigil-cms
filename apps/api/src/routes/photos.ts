 ...
 *
 * POST   /api/v1/sites/:siteId/photos/upload    → bulk upload to Azure Blob + PostgreSQL
 * POST   /api/v1/sites/:siteId/photos/curate    → Gemini vision AI curation
 * GET    /api/v1/sites/:siteId/photos           → list all photos for site
 * PATCH  /api/v1/sites/:siteId/photos/:id       → update selection
 * DELETE /api/v1/sites/:siteId/photos/cleanup   → remove stale records
...
router.post('/upload', upload.array('photos', 100), async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    for (const id of photoIds) {
      ...
      try {
        ...
        console.warn(`[photos/curate] Blob missing for photo ${id}, skipping`);
      }
    }
...
Return ONLY valid JSON matching this schema:
{
  ...
  "summary": "Selected X of Y photos focusing on..."
    ...
    try {
      ...
      console.error('[photos/curate] Failed to parse Gemini response:', responseText.slice(0, 500));
    ...
    console.error('[photos/curate] Error:', msg);
    res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: msg } });
  }
});
