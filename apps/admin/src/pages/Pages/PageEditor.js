...
import { themePresets } from '@netrun-cms/theme';
 ...
 */
function EditorContent({ activeTab, setActiveTab, blocks, setBlocks, selectedBlockId, setSelectedBlockId, handleDeleteBlock, handleToggleBlockVisibility, handleMoveBlock, handleBlockContentChange, handleAddBlock, formData, handleChange, }) {
    return (_jsxs(Tabs, { value: activeTab, onValueChange: setActiveTab, children: [_jsxs(TabsList, { children: [_jsx(TabsTrigger, { value: "content", children: "Content Blocks" }), _jsx(TabsTrigger, { value: "seo", children: "SEO" })] }), _jsxs(TabsContent, { value: "content", className: "space-y-4 mt-4", children: [blocks.length > 0 ? (_jsx(BlockGrid, { blocks: blocks, selectedBlockId: selectedBlockId, onSelect: setSelectedBlockId, onReorder: setBlocks, onResize: (blockId, colSpan) => {
                            setBlocks((prev) => prev.map((b) => b.id === blockId ? { ...b, colSpan } : b));
                        }, onEdit: (id) => setSelectedBlockId(selectedBlockId === id ? null : id), onDelete: (id) => {
                            handleDeleteBlock(id);
                            if (selectedBlockId === id)
                                setSelectedBlockId(null);
                        }, onToggleVisibility: handleToggleBlockVisibility, renderBlock: (block, isSelected) => (_jsx(BlockPreview, { block: block, isSelected: isSelected, onEdit: () => setSelectedBlockId(selectedBlockId === block.id ? null : block.id), onDelete: () => {
                                handleDeleteBlock(block.id);
                                if (selectedBlockId === block.id)
                                    setSelectedBlockId(null);
                            }, onToggleVisibility: () => handleToggleBlockVisibility(block.id), onMoveUp: () => handleMoveBlock(block.id, 'up'), onMoveDown: () => handleMoveBlock(block.id, 'down') })) })) : (_jsx(Card, { children: _jsxs(CardContent, { className: "flex flex-col items-center justify-center py-12", children: [_jsx("div", { className: "flex h-16 w-16 items-center justify-center rounded-full bg-muted", children: _jsx(LayoutGrid, { className: "h-8 w-8 text-muted-foreground" }) }), _jsx("h3", { className: "mt-4 text-lg font-semibold", children: "No content blocks yet" }), _jsx("p", { className: "mt-2 text-center text-sm text-muted-foreground", children: "Start building your page by adding content blocks" })] }) })), selectedBlockId && (() => {
                        const selectedBlock = blocks.find((b) => b.id === selectedBlockId);
                        if (!selectedBlock)
                            return null;
                        return (_jsxs(Card, { className: "border-primary/30", children: [_jsx(CardHeader, { className: "pb-3", children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs(CardTitle, { className: "text-base", children: ["Edit ", blockTypes.find((b) => b.type === selectedBlock.type)?.label || selectedBlock.type, " Content"] }), _jsx(Button, { variant: "ghost", size: "icon", className: "h-7 w-7", onClick: () => setSelectedBlockId(null), children: _jsx(X, { className: "h-4 w-4" }) })] }) }), _jsx(CardContent, { children: _jsx(BlockContentEditor, { blockType: selectedBlock.type, content: selectedBlock.content, onChange: (content) => handleBlockContentChange(selectedBlock.id, content) }) })] }));
                    })(), _jsx(AddBlockButton, { onAdd: handleAddBlock })] }), _jsx(TabsContent, { value: "seo", className: "space-y-4 mt-4", children: _jsxs(Card, { children: [_jsxs(CardHeader, { children: [_jsx(CardTitle, { children: "SEO Settings" }), _jsx(CardDescription, { children: "Search engine optimization for this page" })] }), _jsxs(CardContent, { className: "space-y-4", children: [_jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "metaTitle", children: "Meta Title" }), _jsx(Input, { id: "metaTitle", value: formData.metaTitle, onChange: (e) => handleChange('metaTitle', e.target.value), placeholder: formData.title || 'Page Title', maxLength: 60 }), _jsxs("p", { className: "text-xs text-muted-foreground", children: [formData.metaTitle.length, "/60 characters"] })] }), _jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "metaDescription", children: "Meta Description" }), _jsx(Textarea, { id: "metaDescription", value: formData.metaDescription, onChange: (e) => handleChange('metaDescription', e.target.value), placeholder: "A compelling description for search results", rows: 3, maxLength: 160 }), _jsxs("p", { className: "text-xs text-muted-foreground", children: [formData.metaDescription.length, "/160 characters"] })] }), _jsxs("div", { className: "space-y-2", children: [_jsx(Label, { htmlFor: "ogImageUrl", children: "Social Image URL" }), _jsx(Input, { id: "ogImageUrl", value: formData.ogImageUrl, onChange: (e) => handleChange('ogImageUrl', e.target.value), placeholder: "/images/og-image.jpg" }), _jsx("p", { className: "text-xs text-muted-foreground", children: "Image shown when shared on social media (1200x630px recommended)" })] }), _jsx(Separator, {}), _jsxs("div", { className: "rounded-lg border bg-muted/50 p-4", children: [_jsx("p", { className: "mb-2 text-sm font-medium", children: "Search Preview" }), _jsxs("div", { className: "space-y-1", children: [_jsx("p", { className: "text-lg text-blue-600 hover:underline", children: formData.metaTitle || formData.title || 'Page Title' }), _jsxs("p", { className: "text-sm text-green-700", children: ["netrunsystems.com/", formData.slug || 'page-slug'] }), _jsx("p", { className: "text-sm text-muted-foreground", children: formData.metaDescription || 'Meta description will appear here...' })] })] })] })] }) })] }));
}
export function PageEditor() {
    ...
    const [activeThemePreset, setActiveThemePreset] = useState(null);
    useEffect(() => {
        if (!siteId)
            return;
        api.get('/sites/' + siteId + '/themes/active').then((res) => {
            ...
            if (theme && theme.baseTheme) {
                const preset = themePresets.find((p) => p.id === theme.baseTheme);
                if (preset)
