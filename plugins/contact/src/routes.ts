...
/**
 * Contact Plugin Routes
 *
 * Admin routes (mounted under /api/v1/sites/:siteId/contacts):
 *   GET    /              — list submissions (filter by ?status=&type=)
 *   GET    /stats         — counts grouped by status
 *   PATCH  /:id           — update status/notes
 *   DELETE /:id           — delete submission
 *
 * Public routes (mounted under /api/v1/public/contact/:siteSlug):
 *   POST   /              — submit contact form (general, booking, press)
 *
...
export function createAdminRoutes(db: DrizzleClient): Router {
  ...
  /** GET /stats — submission counts by status */
  ...
  router.patch('/:id', async (req: Request, res: Response) => {
    ...
    const result = await d.update(contactSubmissions)
      .set(updates)
  ...
  /** DELETE /:id — remove submission */
  router.delete('/:id', async (req: Request, res: Response) => {
    const { siteId, id } = req.params;

    await d.delete(contactSubmissions)
      .where(and(eq(contactSubmissions.id, id), eq(contactSubmissions.siteId, siteId)));
...
export function createPublicRoutes(db: DrizzleClient): Router {
  ...
  router.post('/', async (req: Request, res: Response) => {
    ...
    // Save to database
    await d.insert(contactSubmissions).values({
      siteId: site.id,
      name,
      email,
    ...
    try {
      ...
      if (connStr) {
        const [profile] = await d.select({ bookingEmail: artistProfiles.bookingEmail, managementEmail: artistProfiles.managementEmail })
