...
/**
 * Advisor API Routes (plugin version)
 *
 ...
 * Auth is handled by the API integration layer, not this plugin.
 */
...
export function createRoutes(_db: DrizzleClient, logger: PluginLogger): Router {
  const router = Router();
  ...
  router.post('/chat', async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error({ err: errorMessage }, '[advisor/chat] Error');
  ...
  router.post('/documents', upload.single('file'), async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error({ err: errorMessage }, '[advisor/documents] Upload error');
      res.status(500).json({
        success: false,
  ...
  router.delete('/documents/:fileId', async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error({ err: errorMessage }, '[advisor/documents] Delete error');
      res.status(500).json({
        success: false,
        error: { code: 'INTERNAL_ERROR', message: errorMessage },
      });
  ...
  router.post('/tts', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const audioPart = (response.candidates?.[0]?.content?.parts as any[])?.find(
        (p: any) => p.inlineData,
      );
      ...
      logger.error({ err: errorMessage }, '[advisor/tts] Error');
      res.status(500).json({
        success: false,
        error: { code: 'INTERNAL_ERROR', message: errorMessage },
      });
