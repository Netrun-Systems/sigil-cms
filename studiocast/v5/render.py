#!/usr/bin/env python3
"""Sigil CMS Feature Tour v5 — Render video from screenshots + podcast script."""
...
def tts(text, speaker, out):
    ...
    if r.status_code != 200: raise RuntimeError(f"TTS {r.status_code}: {r.text[:200]}")
...
final = Path(__file__).parent / "sigil-tour-v5.mp4"
