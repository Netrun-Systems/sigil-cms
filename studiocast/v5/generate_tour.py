#!/usr/bin/env python3
"""
Sigil CMS Feature Tour v5 — Generate 22 screenshots.

Changes from v4:
1. Updated stats: 48 admin pages, 43 detail pages
2. Feature/Plugin cards show thumbnail images and "Learn more" links
3. New scene 8: Knowledge Base + Detail Page demo (e.g., /plugins/resonance)
4. New scene 22: Lightbox overlay demo
5. Content-aware scroll heights (no extra padding)
6. "Sijil" pronunciation in all dialogue
7. Tour order matches sidebar (22 scenes total)
"""
...
OUT_DIR = "/data/workspace/github/netrun-cms/studiocast/v5/screenshots/"
V5_DIR = Path("/data/workspace/github/netrun-cms/studiocast/v5")
...
def draw_logo(draw, x=20, y=15):
    ...
    # Angular S shape (white geometric S)
    draw.polygon([(x + 8, y + 6), (x + 27, y + 6), (x + 27, y + 10), (x + 8, y + 10)], fill="#ffffff")
    draw.polygon([(x + 8, y + 10), (x + 12, y + 10), (x + 12, y + 20), (x + 8, y + 20)], fill="#ffffff")
    draw.polygon([(x + 8, y + 18), (x + 27, y + 18), (x + 27, y + 22), (x + 8, y + 22)], fill="#ffffff")
    draw.polygon([(x + 23, y + 20), (x + 27, y + 20), (x + 27, y + 32), (x + 23, y + 32)], fill="#ffffff")
    draw.polygon([(x + 8, y + 30), (x + 27, y + 30), (x + 27, y + 34), (x + 8, y + 34)], fill="#ffffff")
...
def draw_feature_card(draw, x, y, w, h, title, desc, color=PRIMARY, has_image=True):
    """Draw a feature card with thumbnail image placeholder and Learn more link."""
    rr(draw, [x, y, x + w, y + h], fill=CARD_BG, outline=CARD_BORDER, radius=10)
    if has_image:
        # Thumbnail placeholder
        img_h = h - 80
        rr(draw, [x + 10, y + 10, x + w - 10, y + 10 + img_h], fill="#1a1a1a", radius=6)
        # Placeholder icon in center of thumbnail
        cx = x + w // 2
        cy_img = y + 10 + img_h // 2
        draw.ellipse([cx - 14, cy_img - 14, cx + 14, cy_img + 14], fill=color, outline=None)
    text_y = y + h - 68 if has_image else y + 14
    draw.text((x + 14, text_y), title, fill=TEXT_PRIMARY, font=F_BODY_BOLD)
    draw.text((x + 14, text_y + 20), desc[:int(w / 7)], fill=TEXT_SECONDARY, font=F_SMALL)
    draw.text((x + 14, text_y + 40), "Learn more \u2192", fill=PRIMARY, font=F_SMALL)
...
def scene_01_landing():
    ...
    d.text((W // 2 - 380, hero_y + 140), sub, fill=TEXT_SECONDARY, font=F_HERO_SUB)
    ...
    stats = [("22", "Plugins"), ("70+", "Google Fonts"), ("48", "Admin Pages"), ("$0", "Self-Host")]
    sx2 = 280
    ...
    # Feature grid with thumbnail images and "Learn more" links
    fy = stats_y + 110
    feat_cards = [
        ("Block Editor", "Composable content blocks"),
        ("Design Playground", "70+ fonts, spacing controls"),
        ("Plugin Marketplace", "22 plugins, zero-config"),
        ("Resonance Analytics", "Block-level heatmaps"),
        ("Commerce Suite", "Stripe, Printful, PayPal"),
        ("Community Forum", "Gated discussions, rep"),
    ]
    card_w = 480
    card_h = 150
    col = 0
    ...
    for title, desc in feat_cards:
        fx = 200 + col * (card_w + 20)
        draw_feature_card(d, fx, row_y, card_w, card_h, title, desc)
        col += 1
        if col >= 3:
            col = 0
            row_y += card_h + 12
...
def scene_05_design():
    ...
    cy += 32
    shapes = [("Square", 2, False), ("Rounded", 8, False), ("Pill", 25, True)]
    for si, (shape, rad, selected) in enumerate(shapes):
        bx2 = CONTENT_X + si * 180
...
# ============================================================
# SCENE 8: Knowledge Base + Detail Page Demo (static)
# ============================================================
def scene_08_knowledge():
    img, d = new_image()
    draw_sidebar(d, "Knowledge Base")
    cy = draw_topbar(d, "Knowledge Base & Detail Pages", "Admin / Knowledge Base")

    # Left half: KB article list
    kb_w = (CONTENT_W - 30) // 2
    d.text((CONTENT_X, cy), "Knowledge Base", fill=TEXT_PRIMARY, font=F_HEADING)
    pill(d, [CONTENT_X + 180, cy, CONTENT_X + 280, cy + 20], INFO, "43 Detail Pages", text_color="#fff")
    cy += 32

    articles = [
        {"title": "Getting Started with Sigil CMS", "cat": "Guides", "views": 4820, "helpful": 94},
        {"title": "Plugin Development API Reference", "cat": "API Docs", "views": 3240, "helpful": 91},
        {"title": "Multi-Tenant Architecture", "cat": "Architecture", "views": 2680, "helpful": 88},
        {"title": "Stripe Integration Guide", "cat": "Plugins", "views": 1950, "helpful": 85},
        {"title": "Design Playground Themes", "cat": "Guides", "views": 1740, "helpful": 92},
    ]

    for art in articles:
        rr(d, [CONTENT_X, cy, CONTENT_X + kb_w, cy + 64], fill=CARD_BG, outline=CARD_BORDER, radius=10)
        d.text((CONTENT_X + 16, cy + 8), art["title"][:32], fill=TEXT_PRIMARY, font=F_BODY_BOLD)
        pill(d, [CONTENT_X + 16, cy + 32, CONTENT_X + 16 + len(art["cat"]) * 7 + 16, cy + 48], INFO, art["cat"], text_color="#fff")
        d.text((CONTENT_X + 16 + len(art["cat"]) * 7 + 26, cy + 34), f"{art['views']:,} views  |  {art['helpful']}% helpful", fill=TEXT_SECONDARY, font=F_SMALL)
        cy += 72

    # Right half: Detail page preview (/plugins/resonance)
    detail_x = CONTENT_X + kb_w + 30
    detail_w = CONTENT_W - kb_w - 30
    detail_y = 90

    rr(d, [detail_x, detail_y, detail_x + detail_w, detail_y + 900], fill="#0e0e0e", outline=PRIMARY, radius=12, width=2)
    d.text((detail_x + 16, detail_y + 12), "Detail Page Preview: /plugins/resonance", fill=TEXT_MUTED, font=F_SMALL_BOLD)
    d.line([detail_x + 10, detail_y + 36, detail_x + detail_w - 10, detail_y + 36], fill=CARD_BORDER, width=1)

    # Hero section
    rr(d, [detail_x + 16, detail_y + 48, detail_x + detail_w - 16, detail_y + 200], fill="#1a2e27", radius=8)
    d.text((detail_x + 36, detail_y + 70), "Resonance Analytics", fill=PRIMARY, font=font(20, bold=True))
    d.text((detail_x + 36, detail_y + 100), "Block-level content analytics with", fill=TEXT_SECONDARY, font=F_BODY)
    d.text((detail_x + 36, detail_y + 120), "AI-powered optimization suggestions.", fill=TEXT_SECONDARY, font=F_BODY)
    rr(d, [detail_x + 36, detail_y + 152, detail_x + 176, detail_y + 180], fill=PRIMARY, radius=16)
    d.text((detail_x + 52, detail_y + 158), "Get Started", fill="#0a0a0a", font=F_SMALL_BOLD)

    # Markdown content
    md_y = detail_y + 216
    d.text((detail_x + 16, md_y), "Overview", fill=TEXT_PRIMARY, font=F_SUBHEADING)
    md_y += 24
    d.text((detail_x + 16, md_y), "Resonance provides engagement heatmaps for", fill=TEXT_SECONDARY, font=F_SMALL)
    md_y += 18
    d.text((detail_x + 16, md_y), "every content block on your site. Powered by", fill=TEXT_SECONDARY, font=F_SMALL)
    md_y += 18
    d.text((detail_x + 16, md_y), "Intersection Observer with zero cookies.", fill=TEXT_SECONDARY, font=F_SMALL)
    md_y += 30

    # Capabilities grid
    d.text((detail_x + 16, md_y), "Capabilities", fill=TEXT_PRIMARY, font=F_SUBHEADING)
    md_y += 24
    caps = [("Heatmaps", SUCCESS), ("A/B Testing", INFO), ("AI Suggestions", PURPLE), ("GDPR Compliant", PRIMARY)]
    cap_w = (detail_w - 52) // 2
    for ci, (cap_name, cap_color) in enumerate(caps):
        cx = detail_x + 16 + (ci % 2) * (cap_w + 10)
        cy2 = md_y + (ci // 2) * 44
        rr(d, [cx, cy2, cx + cap_w, cy2 + 36], fill=CARD_BG, outline=cap_color, radius=6, width=1)
        d.ellipse([cx + 10, cy2 + 12, cx + 22, cy2 + 24], fill=cap_color)
        d.text((cx + 28, cy2 + 10), cap_name, fill=TEXT_PRIMARY, font=F_SMALL)
    md_y += 100

    # CTA
    rr(d, [detail_x + 16, md_y, detail_x + detail_w - 16, md_y + 60], fill="#1a2e27", radius=8)
    d.text((detail_x + 36, md_y + 10), "Ready to optimize your content?", fill=TEXT_PRIMARY, font=F_BODY_BOLD)
    rr(d, [detail_x + 36, md_y + 34, detail_x + 176, md_y + 54], fill=PRIMARY, radius=12)
    d.text((detail_x + 52, md_y + 38), "Enable Plugin", fill="#0a0a0a", font=F_SMALL_BOLD)
...
# ============================================================
# SCENE 10: Commerce -- Store + Merch + PayPal (scroll)
# ============================================================
...
# ============================================================
# SCENE 14: Netrun Platform (scroll)
# ============================================================
...
# ============================================================
# SCENE 20: Plugin Marketplace (scroll) -- with thumbnail images
# ============================================================
def scene_20_marketplace():
    ...
    col2 = 0
    row_y = cy
    card_h = 180  # Taller cards for thumbnail images
    for pname, pcat, active in plugins:
        ...
        rr(d, [px + 12, row_y + 12, px + col_w - 12, row_y + 70], fill="#1a1a1a", radius=6)
        # Icon in thumbnail center
        thumb_cx = px + col_w // 2
        d.ellipse([thumb_cx - 12, row_y + 28, thumb_cx + 12, row_y + 52], fill=ic)

        d.text((px + 16, row_y + 80), pname[:22], fill=TEXT_PRIMARY, font=F_BODY_BOLD)
        pill(d, [px + 16, row_y + 100, px + 16 + len(pcat) * 7 + 16, row_y + 116], ic, pcat, text_color="#0a0a0a")
        if active:
            pill(d, [px + col_w - 64, row_y + 78, px + col_w - 12, row_y + 94], SUCCESS, "Active", text_color="#fff")

        # Learn more link
        d.text((px + 16, row_y + 126), "Learn more \u2192", fill=PRIMARY, font=F_SMALL)

        rr(d, [px + 16, row_y + card_h - 34, px + col_w - 16, row_y + card_h - 12], fill=None, outline=PRIMARY if active else CARD_BORDER, radius=12, width=1)
        btn_text = "Configure" if active else "Activate"
        d.text((px + col_w // 2 - 30, row_y + card_h - 30), btn_text, fill=PRIMARY if active else TEXT_MUTED, font=F_SMALL)
        col2 += 1
        if col2 >= 3:
            col2 = 0
            row_y += card_h + 16
...
def scene_21_pos():
    ...
    cat_colors_pos = {"merch": PINK, "digital": INFO, "service": PURPLE, "event": WARNING}
    cols2 = 3
    ...
    for i, (pname, price, cat, in_stock) in enumerate(products):
        ...
        d.ellipse([px + 14, py + 12, px + 24, py + 22], fill=cat_colors_pos[cat])
...
# ============================================================
# SCENE 22: Lightbox + Detail Page Experience (static)
# ============================================================
def scene_22_lightbox():
    img, d = new_image()

    # Dark overlay background (simulating lightbox)
    d.rectangle([0, 0, W, H_STD], fill="#000000")
    # Semi-transparent overlay effect (darker corners)
    for x_offset in range(0, 60, 2):
        opacity = int(40 - x_offset * 0.5)
        if opacity > 0:
            color = f"#{opacity:02x}{opacity:02x}{opacity:02x}"
            d.rectangle([x_offset, x_offset, W - x_offset, H_STD - x_offset], outline=color)

    # Close button (X) top right
    close_x = W - 60
    close_y = 20
    d.ellipse([close_x, close_y, close_x + 40, close_y + 40], fill="#333333")
    d.text((close_x + 12, close_y + 8), "X", fill=TEXT_PRIMARY, font=F_BODY_BOLD)

    # Lightbox content: zoomed detail page mockup
    lb_x = 160
    lb_y = 60
    lb_w = W - 320
    lb_h = H_STD - 120
    rr(d, [lb_x, lb_y, lb_x + lb_w, lb_y + lb_h], fill="#0e0e0e", outline=PRIMARY, radius=16, width=3)

    # Inner detail page: /plugins/resonance (zoomed view)
    # Hero
    rr(d, [lb_x + 24, lb_y + 24, lb_x + lb_w - 24, lb_y + 200], fill="#1a2e27", radius=12)
    # S logo in hero
    logo_x = lb_x + 50
    logo_y = lb_y + 44
    rr(d, [logo_x, logo_y, logo_x + 40, logo_y + 46], fill=PRIMARY, radius=10)
    lsx, lsy = logo_x + 8, logo_y + 6
    d.polygon([(lsx, lsy), (lsx + 22, lsy), (lsx + 22, lsy + 4), (lsx, lsy + 4)], fill="#ffffff")
    d.polygon([(lsx, lsy + 4), (lsx + 4, lsy + 4), (lsx + 4, lsy + 16), (lsx, lsy + 16)], fill="#ffffff")
    d.polygon([(lsx, lsy + 14), (lsx + 22, lsy + 14), (lsx + 22, lsy + 18), (lsx, lsy + 18)], fill="#ffffff")
    d.polygon([(lsx + 18, lsy + 16), (lsx + 22, lsy + 16), (lsx + 22, lsy + 28), (lsx + 18, lsy + 28)], fill="#ffffff")
    d.polygon([(lsx, lsy + 26), (lsx + 22, lsy + 26), (lsx + 22, lsy + 30), (lsx, lsy + 30)], fill="#ffffff")

    d.text((lb_x + 100, lb_y + 56), "Resonance Analytics", fill=PRIMARY, font=font(28, bold=True))
    d.text((lb_x + 100, lb_y + 96), "Block-level content analytics with AI-powered", fill=TEXT_SECONDARY, font=F_BODY)
    d.text((lb_x + 100, lb_y + 116), "optimization suggestions and A/B testing.", fill=TEXT_SECONDARY, font=F_BODY)
    rr(d, [lb_x + 100, lb_y + 150, lb_x + 260, lb_y + 180], fill=PRIMARY, radius=18)
    d.text((lb_x + 122, lb_y + 156), "Get Started", fill="#0a0a0a", font=F_BODY_BOLD)
    pill(d, [lb_x + 280, lb_y + 152, lb_x + 380, lb_y + 172], CARD_BORDER, "Analytics", text_color=TEXT_SECONDARY)

    # Content body
    body_y = lb_y + 220
    d.text((lb_x + 40, body_y), "Overview", fill=TEXT_PRIMARY, font=F_HEADING)
    body_y += 28
    for line in ["Resonance provides engagement heatmaps for every content block on your",
                  "site. The 2KB tracking snippet uses Intersection Observer with zero cookies",
                  "and collects no PII. Blocks scoring below 40 trigger AI suggestions."]:
        d.text((lb_x + 40, body_y), line, fill=TEXT_SECONDARY, font=F_BODY)
        body_y += 22
    body_y += 16

    # Capabilities grid
    d.text((lb_x + 40, body_y), "Capabilities", fill=TEXT_PRIMARY, font=F_HEADING)
    body_y += 28
    caps = [
        ("Block Heatmaps", "Visual engagement scoring per block", SUCCESS),
        ("A/B Testing", "Clone blocks, split traffic, measure", INFO),
        ("AI Suggestions", "Gemini-powered content optimization", PURPLE),
        ("GDPR Compliant", "No cookies, no PII, no ext scripts", PRIMARY),
        ("Dashboard", "Composite scores and trend graphs", WARNING),
        ("Export Reports", "CSV and JSON engagement exports", TEXT_SECONDARY),
    ]
    cap_w = (lb_w - 100) // 3
    for ci, (cap_name, cap_desc, cap_color) in enumerate(caps):
        cx = lb_x + 40 + (ci % 3) * (cap_w + 16)
        cy2 = body_y + (ci // 3) * 80
        rr(d, [cx, cy2, cx + cap_w, cy2 + 70], fill=CARD_BG, outline=cap_color, radius=8, width=1)
        d.ellipse([cx + 12, cy2 + 12, cx + 24, cy2 + 24], fill=cap_color)
        d.text((cx + 32, cy2 + 10), cap_name, fill=TEXT_PRIMARY, font=F_BODY_BOLD)
        d.text((cx + 12, cy2 + 36), cap_desc[:int(cap_w / 7)], fill=TEXT_SECONDARY, font=F_SMALL)
    body_y += 180

    # CTA at bottom
    rr(d, [lb_x + 40, body_y, lb_x + lb_w - 40, body_y + 70], fill="#1a2e27", radius=10)
    d.text((lb_x + 60, body_y + 12), "Ready to optimize your content performance?", fill=TEXT_PRIMARY, font=F_BODY_BOLD)
    rr(d, [lb_x + 60, body_y + 38, lb_x + 220, body_y + 62], fill=PRIMARY, radius=14)
    d.text((lb_x + 78, body_y + 44), "Enable Plugin", fill="#0a0a0a", font=F_SMALL_BOLD)
    d.text((lb_x + 240, body_y + 44), "View Documentation", fill=PRIMARY, font=F_SMALL)

    # Lightbox navigation hint
    d.text((W // 2 - 100, H_STD - 40), "ESC to close  |  Click outside to dismiss", fill=TEXT_MUTED, font=F_SMALL)

    img.save(OUT_DIR + "22-lightbox.png", optimize=True)
    print("  22-lightbox.png")
...
# ============================================================
def generate_all():
    print("Generating Sigil CMS Feature Tour v5 -- 22 screenshots\n")
    scene_01_landing()
    scene_02_dashboard()
    scene_03_sites()
    scene_04_editor()
